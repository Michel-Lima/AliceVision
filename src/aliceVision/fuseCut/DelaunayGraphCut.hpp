...
public:
    ...
    mesh::Mesh* createTetrahedralMesh(bool filter = true, const std::function<float(const GC_cellInfo&)> getScore = [](const GC_cellInfo& c) { return c.emptinessScore; }, const float& downscaleFactor = 0.95f) const;
